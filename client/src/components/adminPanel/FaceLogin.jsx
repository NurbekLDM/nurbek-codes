...
export default function FaceLogin() {
        ...
        try {
          ...
          const faceMeshInstance = ml5.default.faceMesh(
            ...
            () => {
              ...
              img.onload = () => {
                console.log('Image loaded:', { src: img.src, width: img.width, height: img.height });
                faceMeshInstance.detect(img, (results) => {
                  console.log('Raw faceMesh.detect results:', results);
                  if (results && results.length > 0 && Array.isArray(results[0].keypoints)) {
                    ...
                    console.log('✅ Normalized reference keypoints:', normalizedKeypoints);
                  } else {
                    console.error('❌ No face detected or keypoints massiv emas in reference image!');
                  }
                });
              };
              img.onerror = () => {
                console.error('❌ Reference image failed to load:', referenceImageURL);
              };
            }
          );
        } catch (error) {
  ...
  async function detectFace(faceMeshInstance, videoElement) {
    ...
    try {
      console.log('Video holati:', {
        ...
        playing: videoElement.currentTime > 0 && !videoElement.paused && !videoElement.ended,
      });
      ...
      faceMeshInstance.detect(videoElement, (results) => {
        ...
        if (results && results.length > 0 && Array.isArray(results[0].keypoints)) {
          const normalizedKeypoints = results[0].keypoints.map(kp => ({
            ...
            z: kp.z / Math.max(videoElement.videoWidth, videoElement.videoHeight),
          }));
  ...
  function compareFaces(keypoints1, keypoints2) {
    ...
    for (let i = 0; i < Math.min(keypoints1.length, keypoints2.length); i++) {
      ...
      if (!kp1 || !kp2 || typeof kp1 !== 'object' || typeof kp2 !== 'object') {
        console.error('❌ keypoints1[i] yoki keypoints2[i] obyekt emas', { i, kp1, kp2 });
        return false;
      }

      const x1 = kp1.x, y1 = kp1.y, z1 = kp1.z;
      const x2 = kp2.x, y2 = kp2.y, z2 = kp2.z;
      const pointDistance = Math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2 + (z1 - z2) ** 2);
  ...
  async function handleLogin() {
    try {
      const response = await fetch('https://nurbek-codes-9olu.vercel.app/api/admin/face-login', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className='mt-4 text-center'>
      <p className='text-sm text-gray-500 dark:text-gray-400'>Or sign in using face recognition</p>
      ...
      <p>{isAuthenticated ? '✅ Tizimga kirdingiz!' : isCameraActive ? '⏳ Kamera orqali yuzingizni tasdiqlang' : ''}</p>
    </div>
  );
}